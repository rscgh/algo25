...
class FriendsDatasetTest(Dataset):
    def __init__(self, root, modalities=["fmri", "video"], image_transform=None, tr=1.49, seasons=["figures"],
                 subjects=[1,2,3,5], timesample=1, fmri_window=1, target_video_len=32, stimulus_window=1, hrf_delay=0,
                 downsampled=False):
        self.root = root
        ...
        from glob import glob
        mv= "movies_224" if self.downsampled else "movies"
        mkv_files = glob(f"{self.root}/algonauts_2025.competitors/stimuli/{mv}/**/**/*.mkv")
        self.epikey_to_mkv = {os.path.splitext(os.path.basename(p))[0]: p for p in mkv_files}

        last_idx = 0
        for subject in subjects:
            fmri = load_fmri(root, subject)
            episodes_in_sets = [key for key in fmri if \
                                np.any([key.startswith(season.split("-")[-1]) \
                                       for season in self.seasons])]

            print(episodes_in_sets)

            #for key in fmri.keys():
            for key in episodes_in_sets:
                curr_fmri = fmri[key]
                fmri_samples = curr_fmri.shape[0]

                #self.fmris.append({"movie": key, "n_samples": fmri_samples, "subject": subject})
        ...
        print("Loaded", len(self.fmris), "movie files, total samples:", self.tot_samples)
    ...
    def get_movie_path(self, movie_name) -> VideoDecoder:
        ...
        if not movie_name in self.epikey_to_mkv.keys():
            print(self.epikey_to_mkv.keys())
            print("missing movie mkv:", movie_name)
        
        episode_path=self.epikey_to_mkv[movie_name]
        if not os.path.exists(episode_path):
            print("doesnt exist:", episode_path)
...
class FriendsDataset(Dataset):
    def __init__(self, root, modalities=["fmri", "video"], image_transform=None, tr=1.49, seasons=[1,2,3,4,5,6],
        ...
        for subject in subjects:
            ...
            episodes_in_sets = [key for key in fmri if np.any([key.startswith(season.split("-")[-1]) for season in self.seasons])]
