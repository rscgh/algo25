...
model_name = checkpoint.split("/")[-1].replace("-", "_") + "_untr"
print(model_name)
...
"""
#Alternative: get untrained model
from transformers import AutoModelForCausalLM, AutoConfig
config = AutoConfig.from_pretrained(checkpoint)
model = AutoModelForCausalLM.from_config(config, torch_dtype=torch.bfloat16).to("cuda").eval()
"""
...
#for i in range(400//3): next(iterator);
...
for j, transcript_file in tqdm(enumerate(all_tsv_files), total=len(all_tsv_files)):
  ...
  fn = f"{actv_dir}/actv-{model_name}-{id}-4L_6tok_last500words.npy"
  ...
  for input_ids, attention_mask in tqdm(dataloader, total=len(dataloader)):
    ...
    layer_idxs = np.linspace(0, len(xhs)-1, 4).round().astype(int)
